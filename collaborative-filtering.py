...
distances, indices = model_knn.kneighbors(
    dataset1.iloc[0, :].values.reshape(1, -1), n_neighbors=10)
similarities = 1-distances.flatten()
print(distances, indices, similarities)
mean_rating = dataset1.iloc[0, :].mean()


sum_wt = np.sum(similarities)-1

for label in predictedJokesLabels['0']:
    prediction = 0
    wtd_sum = 0
    product = 1
    ...
    print('\nPredicted rating for user -> joke %s: %f' % (label, prediction))
